...
fn parse_field(data: &[u8]) -> (Field, &[u8]) {
    ...
    let (fieldvalue, remainder) = match wire_type {
        ...
        WireType::Len => {
            ...
            if len as usize > rem.len() {
                panic!("Not enough bytes for length-delimited field");
            }
            let payload = &rem[..len as usize];
            let rem = &rem[len as usize..];
            (FieldValue::Len(payload), rem)
        }
    };
...
// Каждое поле кодируется как ключ-значение
// ключ в данном случае это энум Variant 
// 0 - VARIANT - nt32, int64, uint32, bool, enum
// 2 - LEN - string

// ключ 
// tag = (field_number << 3) | wire_type
// сдвиг номера поля на 3 и добавляет тип данных в младшие 3 бита

// ключ кодирует номер поля